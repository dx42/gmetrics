 ...
 *   5. Add one to the branch count for each occurrence of the null-safe dereference (e.g. x?.y) or the null-safe indexing (e.g. x?[1]).
 *   6. Add one to the condition count for each use of a conditional operator:
 *      == != <= >= < > <=> =~ ==~ === !== instanceof !instanceof in !in
 *   7. Add one to the condition count for each use of the following keywords:
 *      else case default try catch ?
 *   8. Add one to the condition count for each unary conditional expression. These are cases where
...
class AbcMetric extends AbstractMethodMetric {
    ...
    MetricResult calculate(MethodNode methodNode, SourceCode sourceCode) {
        ...
        if (visitor.visited) {
            ...
            return new AbcMetricResult(this, MetricLevel.METHOD, abcVector, lineNumberForMethod(methodNode, sourceCode))
        }
        return null
    }
